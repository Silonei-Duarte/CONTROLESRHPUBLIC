...
class AbaGraficoFrequencia(QWidget):
    ...
    def update_situacao_filter(self):
        ...
        #  Adicionar opção "Todas as Situações"
    ...
    def plot_graph(self):
        ...
        # Mapeamento das colunas de horas
        colunas_horas = {
        ...
        grouped_local = df_local.groupby("Local")[situacao].sum()
        grouped_local_horas = df_local.groupby("Local")[
            coluna_horas].sum() if coluna_horas and coluna_horas in df_local.columns else pd.Series(
        ...
        grouped_setor = df_setor.groupby(["Local", "Setor"])[situacao].sum()
        grouped_setor_horas = df_setor.groupby(["Local", "Setor"])[
            coluna_horas].sum() if coluna_horas and coluna_horas in df_setor.columns else pd.DataFrame()
        grouped_setor = grouped_setor[grouped_setor > 0].reset_index()
        grouped_setor[situacao] = grouped_setor[situacao].astype(int)
        ...
        grouped_nome = df_nome.groupby("Colaborador")[situacao].sum()
        grouped_nome_horas = df_nome.groupby("Colaborador")[
            coluna_horas].sum() if coluna_horas and coluna_horas in df_nome.columns else pd.Series(
            dtype="timedelta64[ns]")
        ...
        local_data = [
            (val, local, self.COLOR_MAP.get(local, "#000000"), grouped_local_horas.get(local, pd.Timedelta(0)))
            for local, val in grouped_local.items()]

        # Construir o dicionário de horas por setor com segurança
        if isinstance(grouped_setor_horas, pd.DataFrame) and not grouped_setor_horas.empty:
            setor_horas_dict = grouped_setor_horas.set_index(["Local", "Setor"])[coluna_horas].to_dict()
        elif isinstance(grouped_setor_horas, pd.Series):
            setor_horas_dict = grouped_setor_horas.to_dict()
        else:
            setor_horas_dict = {}

        setor_data = [
            (row[situacao], f"{row['Local']} - {row['Setor']}",
             self.SECTOR_COLOR_MAP.get(f"{row['Local']} - {row['Setor']}",
                                       self.SECTOR_COLOR_MAP.get(row['Local'], "#000000")),
             setor_horas_dict.get((row['Local'], row['Setor']), pd.Timedelta(0)))
            for _, row in grouped_setor.iterrows()
        ]


        nome_data = [(val, nome, self.NAME_COLOR, grouped_nome_horas.get(nome, pd.Timedelta(0)))
        ...
        values, labels, colors, horas = zip(*all_data) if all_data else ([], [], [], [])
        horas_formatadas = [self.formatar_horas(h) if isinstance(h, (pd.Timedelta, int, float)) else "00:00:00" for h in
                            horas]
        text_labels = [f"{v} | {h}" if h != "00:00:00" else f"{v}" for v, h in zip(values, horas_formatadas)]
        ...
        max_text = max(len(t) for t in text_labels) if text_labels else 10
        extra_space = max_text * 10
        max_value = max(values) if values else 0
        dynamic_width = min(1800, max(1800, max_value + extra_space))

        fig.update_layout(
            title=f"Ocorrências: {situacao} | Período: {self.periodo}",
            xaxis_title="Quantidade",
            yaxis=dict(autorange="reversed"),
            height=max(500, len(labels) * 30),
            width=dynamic_width,
            margin=dict(l=100, r=150, t=50, b=50),
            xaxis=dict(range=[0, max_value + 30]),
            plot_bgcolor='white',
            paper_bgcolor='white'
        )
    ...
    def plot_all_situations(self):
        """Plota o gráfico exibindo todas as situações separadas por local, setor e colaborador, usando Plotly."""
        if self.filtered_df.empty:
            html_content = "<h3 style='text-align:center;'>Nenhum dado disponível</h3>"
            self.web_view.setHtml(html_content)
            return
        ...
        for _, row in self.filtered_df.iterrows():
            ...
            if local_filtro != "Todas" and local_name != local_filtro:
                continue
            if setor_filtro != "Todas" and setor_filtro != "Não listar" and setor_name != setor_filtro:
                continue
        ...
        extra_space = max_text_length * 11  # Ajuste o multiplicador para garantir o espaço necessário para o texto
        ...
        fig.update_layout(
            title=f"Ocorrências: Todas as Situações | Período: {self.periodo}",
            xaxis_title="Quantidade",
            yaxis=dict(autorange="reversed"),
            height=max(500, len(y_labels) * 30),
            width=dynamic_width,
            margin=dict(l=100, r=200, t=50, b=50),  # margem direita maior
            xaxis=dict(range=[0, max_value + 30]),  # mais espaço no eixo
            plot_bgcolor='white',
            paper_bgcolor='white'
        )
